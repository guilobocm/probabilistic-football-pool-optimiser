...
def test_run_all_and_outputs_schema(tmp_path):
    ...
    assert "champion_probabilities" in sim_data, "Missing champion_probabilities in sim summary."
    assert "semifinal_probabilities" in sim_data, "Missing semifinal_probabilities in sim summary."
    ...
    assert "team_a,team_b,pick_a,pick_b" in lines[0] or "match" in lines[0].lower(), "match_picks.csv has invalid header."
